...
//#endregion
//#region node_modules/long/dist/long.js
...
//#endregion
//#region node_modules/bytebuffer/dist/bytebuffer.js
var require_bytebuffer = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	*/
	(function(global, factory) {
		...
		else if (typeof __require === "function" && typeof module === "object" && module && module["exports"]) module["exports"] = (function() {
			var Long;
			try {
				Long = require_long();
			} catch (e) {}
		...
		*/
		ByteBuffer.wrap = function(buffer, encoding, littleEndian, noAssert) {
			...
			if (typeof buffer === "string") {
				...
				switch (encoding) {
					case "base64": return ByteBuffer.fromBase64(buffer, littleEndian);
					case "hex": return ByteBuffer.fromHex(buffer, littleEndian);
					case "binary": return ByteBuffer.fromBinary(buffer, littleEndian);
					case "utf8": return ByteBuffer.fromUTF8(buffer, littleEndian);
					case "debug": return ByteBuffer.fromDebug(buffer, littleEndian);
		...
		if (Long) {
			...
			*/
			ByteBufferPrototype.writeVarint64 = function(value, offset) {
				...
				offset -= size;
				switch (size) {
					case 10: this.view[offset + 9] = part2 >>> 7 & 1;
					case 9: this.view[offset + 8] = size !== 9 ? part2 | 128 : part2 & 127;
					case 8: this.view[offset + 7] = size !== 8 ? part1 >>> 21 | 128 : part1 >>> 21 & 127;
					case 7: this.view[offset + 6] = size !== 7 ? part1 >>> 14 | 128 : part1 >>> 14 & 127;
					case 6: this.view[offset + 5] = size !== 6 ? part1 >>> 7 | 128 : part1 >>> 7 & 127;
					case 5: this.view[offset + 4] = size !== 5 ? part1 | 128 : part1 & 127;
					case 4: this.view[offset + 3] = size !== 4 ? part0 >>> 21 | 128 : part0 >>> 21 & 127;
					case 3: this.view[offset + 2] = size !== 3 ? part0 >>> 14 | 128 : part0 >>> 14 & 127;
					case 2: this.view[offset + 1] = size !== 2 ? part0 >>> 7 | 128 : part0 >>> 7 & 127;
					case 1: this.view[offset] = size !== 1 ? part0 | 128 : part0 & 127;
				}
				if (relative) {
					this.offset += size;
					return this;
		...
		*/
		ByteBufferPrototype.writeIString = function(str, offset) {
			...
			var start = offset, k = utfx.calculateUTF16asUTF8(stringSource(str), this.noAssert)[1];
			offset += 4 + k;
		...
		*/
		ByteBufferPrototype.writeVString = function(str, offset) {
			...
			var start = offset, k = utfx.calculateUTF16asUTF8(stringSource(str), this.noAssert)[1], l = ByteBuffer.calculateVarint32(k);
			offset += l + k;
			...
			lxiv.decode = function(src, dst) {
				...
				while ((c = src()) !== null) {
					...
					if ((c = src()) !== null) {
						...
						if ((c = src()) !== null) {
							t1 = ain[c];
							if (typeof t1 === "undefined") if (c === 61) break;
							else fail(c);
							dst((t2 & 15) << 4 >>> 0 | (t1 & 60) >> 2);
							if ((c = src()) !== null) {
								t2 = ain[c];
								if (typeof t2 === "undefined") if (c === 61) break;
								else fail(c);
								dst((t1 & 3) << 6 >>> 0 | t2);
							}
						}
...
//#endregion
//#region src/main/preload.js
var { contextBridge, ipcRenderer } = require_electron();
require_bytebuffer();
	...
	ipcRenderer: {
		...
		getAccountDetails() {
			return new Promise((resolve) => {
				...
				ipcRenderer.once("electron-store-getAccountDetails-reply", (evt, message) => {
					resolve(message);
				});
			});
		},
		...
		loginUser(username, password, shouldRemember, authcode, sharedSecret, clientjstoken) {
			console.log(clientjstoken);
			if (authcode == "") authcode = null;
			if (sharedSecret == "") sharedSecret = null;
			if (clientjstoken == "") clientjstoken = null;
			return new Promise((resolve) => {
				ipcRenderer.send("login", username, password, shouldRemember, authcode, sharedSecret, clientjstoken);
				ipcRenderer.once("login-reply", (event, arg) => {
					resolve(arg);
				});
			});
		...
		moveFromStorageUnit(casketID, itemID, fastMode) {
			let storageUnitResponse = new Promise((resolve) => {
				...
				if (fastMode) resolve(fastMode);
				else ipcRenderer.once("removeFromStorageUnit-reply", (event, arg) => {
					resolve(arg);
				});
			});
		...
		moveToStorageUnit(casketID, itemID, fastMode) {
			let storageUnitResponse = new Promise((resolve) => {
				...
				if (fastMode) resolve(fastMode);
				else ipcRenderer.once("moveToStorageUnit-reply", (event, arg) => {
					resolve(arg);
				});
			});
			if (fastMode) return true;
		...
		on(channel, func) {
			if ([
				...
				"cancelQRLogin",
				"qrLogin:show"
			].includes(channel)) ipcRenderer.on(channel, (event, ...args) => func(...args));
		},
		once(channel, func) {
			if ([
				"ipc-example",
				...
				"cancelQRLogin",
				"qrLogin:show"
			].includes(channel)) ipcRenderer.once(channel, (event, ...args) => func(...args));
		}
	},
	store: {
		get(val) {
