...
export default function settingsPage() {
  ...
  // Fast consistent move (dev mode only)
  async function updateFastConsistentMove() {
    const correctValue = !(await window.electron.store.get('fastconsistentmove'));
    setFastConsistentMoveStatus(correctValue);
    await window.electron.store.set('fastconsistentmove', correctValue);
    dispatch(setFastConsistentMove(correctValue));
  }
  const [fastConsistentMoveStatus, setFastConsistentMoveStatus] = useState(settingsData.fastConsistentMove);
                      ...
                      <div className="divide-y divide-gray-200">
                        ...
                        <h3 className="text-lg pt-5 leading-6 font-medium text-gray-900 dark:text-dark-white">
                          ...
                          <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4">
                            ...
                            <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4">
                              <dt className="text-sm font-medium text-gray-900 dark:text-dark-white">
                                Fast consistent move <br />
                                <span className="text-gray-400">
                                  {' '}
                                  Concurrent workers, each waiting for GC ack
                                </span>
                              </dt>
                              <dd className="mt-1 flex text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                <span className="flex-grow"></span>
                                <span className="flex items-center ml-4 flex-shrink-0">
                                  <Switch
                                    checked={fastConsistentMoveStatus}
                                    onChange={() => updateFastConsistentMove()}
                                    className={classNames(
                                      fastConsistentMoveStatus
                                        ? 'bg-indigo-600 dark:bg-indigo-700'
                                        : 'bg-gray-200',
                                      'relative inline-flex mr-3 flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none'
                                    )}
